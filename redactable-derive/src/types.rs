...
mod tests {
    ...
    // These retired classifier probes remain test-only integrity inventory.
    // Production traversal no longer calls either helper: real PhantomData and
    // scalar behavior is resolved by trait/type checking in downstream tests.
