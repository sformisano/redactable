...
use crate::{
    container::{
        ContainerOptions, parse_container_options, parse_json_output,
        reject_field_only_container_attrs,
    },
...
pub(crate) fn expand_with_mode(
    ...
    let output_json = parse_json_output(
        &attrs,
        authenticated_dual || matches!(kind, DeriveKind::Sensitive),
    )?;
    let field_only_attrs: Vec<_> = attrs
        .iter()
        .filter(|attr| !attr.path().is_ident("redactable"))
        .cloned()
        .collect();
    reject_field_only_container_attrs(&field_only_attrs)?;
...
/// `ToRedactedOutput`, production-consistent `Debug`, and — outside dual
...
fn expand_sensitive_display(
    ...
    let ExpansionMode { dual, output_json } = mode;
    ...
    let to_redacted_output_impl = if output_json {
        quote! {}
    } else {
        quote! {
...
/// production-consistent `Debug`, and the slog/tracing integration impls.
fn expand_sensitive(
    ident: Ident,
    generics: Generics,
    data: Data,
    ...
    let crate_root = crate_root();
    let to_redacted_output_impl = if output_json {
        json_output_impl(&ident, &generics, &crate_root)
    } else {
        quote! {}
    };
