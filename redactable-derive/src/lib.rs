...
/// # Generated Impls
///
/// - `ToRedactedOutput`: generated with `#[redactable(output = json)]`; delegates to
///   `redacted_json()` and requires the runtime `json` feature plus `Clone` and `Serialize`.
...
/// including fields omitted from the template. With `#[redactable(output = json)]`,
/// `ToRedactedOutput` selects JSON while `.redacted_display()` retains the template.
