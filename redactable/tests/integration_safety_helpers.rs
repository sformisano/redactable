...
use redactable::{
    NotSensitiveJsonExt, RedactedList, RedactedOutput, RedactedOutputView, ToRedactedOutput,
...
impl ToRedactedOutput for Item<'_> {
    fn to_redacted_output(&self) -> RedactedOutput {
        ...
        if self.index.is_multiple_of(2) {
            UncheckedRedactedSummary::new(format!("item{}", self.index)).to_redacted_output()
        } else {
...
fn selected_json(value: &impl ToRedactedOutput) -> Value {
    match value.to_redacted_output().view() {
        RedactedOutputView::Json(value) => value.clone(),
...
fn list_invokes_included_items_once_in_order_and_never_omitted_items() {
    ...
    assert_eq!(
        selected_json(&list),
        json!({"items":["item0",{"index":1}],"omitted":3})
    );
...
fn list_limits_cover_empty_exact_overlarge_and_maximum() {
    ...
    let empty_summary = [UncheckedRedactedSummary::new(String::new())];
    assert_eq!(
        selected_json(&RedactedList::new(
            &empty_summary,
            NonZeroUsize::new(1).unwrap()
        )),
        json!({"items":[""],"omitted":0})
    );
}
...
mod shape {
    ...
    use redactable::{
        NotSensitiveJsonExt, ToRedactedOutput, UncheckedRedactedSummary, testing::assert_json_shape,
    };
    use serde_json::{Value, json};
    ...
    fn shape(plain: &Value, selected: &Value, opaque: &[&str]) {
        assert_json_shape(
            plain,
            &selected.not_sensitive_json().to_redacted_output(),
            opaque,
        );
    }
    ...
    #[test]
    fn text_output_is_rejected_even_when_empty_or_the_root_is_opaque() {
        for text in ["", "[REDACTED]", "{\"owner\":\"masked\"}"] {
            let summary = UncheckedRedactedSummary::new(text.into()).to_redacted_output();
            assert!(
                catch_unwind(|| assert_json_shape(&json!({"owner":"Ada"}), &summary, &[])).is_err()
            );
            assert!(catch_unwind(|| assert_json_shape(&json!({}), &summary, &[""])).is_err());
        }
    }
