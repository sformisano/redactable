...
use redactable::{
    Pii, Redactable, RedactableMapper, RedactableWithFormatter, RedactableWithMapper,
    RedactedOutputView, Secret, Sensitive, SensitiveDisplay, Token,
};
use serde::Serialize;
...
fn works_with_to_redacted_output() {
    ...
    assert_eq!(
        output.view(),
        RedactedOutputView::Text("login failed for alice [REDACTED]")
    );
}
