...
use redactable::{
    Default, NotSensitiveValue, Redactable, RedactableLeaf, RedactableWithPolicy, RedactedOutput,
    RedactionPolicy, Sensitive, SensitiveValue, TextRedactionPolicy, ToRedactedOutput, Token,
};
#[cfg(feature = "slog")]
use serde::Serialize;

// =============================================================================
// SensitiveValue<T, P> wrapper tests
// =============================================================================

#[test]
fn sensitive_wrapper_from_value() {
    let sensitive = SensitiveValue::<String, Default>::from("password".to_string());
    assert_eq!(sensitive.expose(), "password");
}

#[test]
fn sensitive_wrapper_expose() {
    let sensitive = SensitiveValue::<String, Token>::from("tok_abc123".to_string());
    // expose() gives access to the inner value
    assert_eq!(sensitive.expose().len(), 10);
    assert!(sensitive.expose().starts_with("tok_"));
}

#[test]
fn sensitive_wrapper_expose_mut() {
    let mut sensitive = SensitiveValue::<String, Default>::from("password".to_string());
    sensitive.expose_mut().push_str("123");
    assert_eq!(sensitive.expose(), "password123");
}

#[test]
fn sensitive_wrapper_debug_shows_redacted() {
    let sensitive = SensitiveValue::<String, Default>::from("hunter2".to_string());
...
#[test]
fn sensitive_wrapper_redacted_method() {
    let sensitive = SensitiveValue::<String, Token>::from("sk_live_abc123def".to_string());
    // Token keeps last 4: "sk_live_abc123def" (17 chars) → 13 asterisks + "3def"
    assert_eq!(sensitive.redacted(), "*************3def");
}

#[test]
fn sensitive_wrapper_to_redacted_output() {
    let sensitive = SensitiveValue::<String, Default>::from("secret".to_string());
    assert_eq!(
...
#[test]
fn sensitive_wrapper_in_container_redacts() {
...
#[test]
fn sensitive_wrapper_with_custom_leaf_type() {
    ...
    #[cfg_attr(feature = "slog", derive(Serialize))]
    struct Request {
        user_id: SensitiveValue<UserId, Default>,
    }
...
// =============================================================================
// NotSensitiveValue<T> wrapper tests
// =============================================================================

#[test]
fn not_sensitive_value_from_value() {
    #[derive(Clone, PartialEq, Debug)]
    struct ForeignType {
        data: String,
    }
...
#[test]
fn not_sensitive_value_deref_mut() {
    #[derive(Clone)]
    struct ForeignType {
        value: i32,
    }
...
#[test]
fn not_sensitive_value_debug_shows_inner() {
    #[allow(dead_code)]
    #[derive(Clone, Debug)]
    struct ForeignType {
        data: String,
...
#[test]
fn not_sensitive_value_passes_through_in_container() {
    ...
    // foreign should pass through unchanged
    assert_eq!(redacted.foreign.timeout, 30);
    assert_eq!(redacted.foreign.retries, 3);
}

#[test]
fn not_sensitive_value_does_not_walk_nested_sensitive() {
    // Even if the inner type has sensitive fields, NotSensitiveValue does NOT walk them
    #[derive(Clone, Sensitive)]
    #[cfg_attr(feature = "slog", derive(Serialize))]
    ...
    #[cfg_attr(feature = "slog", derive(Serialize))]
    struct Outer {
        // This will NOT redact password because NotSensitiveValue doesn't walk
...
// =============================================================================
// Orphan rule workaround tests (RedactableWithPolicy)
// =============================================================================
...
// Implement RedactableWithPolicy for the "foreign" type using our local policy
...
#[test]
fn orphan_rule_workaround_sensitive_wrapper() {
    // Use SensitiveValue with our local policy to redact the foreign type
    let wrapped =
        SensitiveValue::<ForeignId, ForeignIdPolicy>::from(ForeignId("external_12345".to_string()));
    assert_eq!(wrapped.redacted(), "**********2345");
}

#[test]
fn orphan_rule_workaround_in_container() {
    #[derive(Clone, Sensitive)]
    #[cfg_attr(feature = "slog", derive(Serialize))]
    struct Integration {
        // Foreign type wrapped with SensitiveValue and local policy
...
#[test]
fn orphan_rule_workaround_to_redacted_output() {
    let wrapped =
...
#[test]
fn orphan_rule_reuse_builtin_policy_logic() {
    // Demonstrate reusing Token's policy logic for a local policy
    struct MyTokenPolicy;
    impl RedactionPolicy for MyTokenPolicy {
        fn policy() -> TextRedactionPolicy {
            Token::policy() // Reuse Token's policy
        }
    }
    ...
    // Token policy keeps last 4 chars
    assert_eq!(wrapped.redacted(), "***********cret");
}

// =============================================================================
// Combined wrapper scenarios
// =============================================================================

#[test]
fn mixed_wrappers_in_same_container() {
    #[derive(Clone, Debug)]
    #[cfg_attr(feature = "slog", derive(Serialize))]
...
#[test]
fn nested_sensitive_in_option() {
    #[derive(Clone, Sensitive)]
    #[cfg_attr(feature = "slog", derive(Serialize))]
    struct MaybeSensitive {
        sensitive: Option<SensitiveValue<String, Default>>,
    }
...
// =============================================================================
// Verify derive macro and wrapper type coexistence
// =============================================================================
...
fn not_sensitive_derive_and_wrapper_coexist() {
    ...
    // NotSensitive derive macro for types we own
    ...
    // NotSensitiveValue wrapper for "foreign" types
    #[derive(Clone, Debug)]
    #[cfg_attr(feature = "slog", derive(Serialize))]
    struct ForeignData {
        value: i32,
    ...
    #[cfg_attr(feature = "slog", derive(Serialize))]
    struct Combined {
        public: PublicData,                      // Uses NotSensitive derive
        foreign: NotSensitiveValue<ForeignData>, // Uses NotSensitiveValue wrapper
        #[sensitive(Default)]
        password: String,
    }
