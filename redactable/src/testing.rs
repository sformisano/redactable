//! Runtime assertions for selected JSON output.
//!
//! Requires both `testing` and `json`. These helpers never enable raw Debug.
...
use crate::{RedactedOutput, RedactedOutputView};

/// Asserts that selected JSON has the same shape as `plain`, except at opaque nodes.
///
...
/// # Panics
///
/// Panics for malformed paths, invalid array tokens, non-JSON output, or a
/// shape mismatch. Diagnostics identify the path without printing values.
///
/// ```
/// # #![allow(hidden_glob_reexports)]
/// # pub use redactable::*;
/// use redactable::{RedactedOutputView, Secret, Sensitive, ToRedactedOutput, testing::assert_json_shape};
/// use serde_json::json;
///
...
/// let output = decision.to_redacted_output();
/// assert_json_shape(&plain, &output, &[]);
/// assert_eq!(output.view(), RedactedOutputView::Json(&expected));
/// # }
/// ```
#[track_caller]
pub fn assert_json_shape(plain: &Value, output: &RedactedOutput, opaque_paths: &[&str]) {
    let paths: Vec<_> = opaque_paths
        .iter()
        .map(|path| parse_pointer(path))
        .collect();
    let RedactedOutputView::Json(selected) = output.view() else {
        panic!("JSON shape mismatch at root: expected JSON output");
    };
    for path in &paths {
